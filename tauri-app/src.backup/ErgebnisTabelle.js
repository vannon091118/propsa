...
exports.ErgebnisTabelle = void 0;
...
const KOPFZELLE = "sticky top-0 z-10 border-b border-white/10 bg-kopf/80 px-2.5 py-2 text-left text-[12px] font-semibold uppercase tracking-[0.03em] text-leise backdrop-blur";
...
function groessteZeilen(ergebnis) {
    let groesste = 1;
...
function ErgebnisTabelleBasis({ ergebnis }) {
    const [alleZeigen, setAlleZeigen] = (0, react_1.useState)(false);
    if (ergebnis.dateien.length === 0) {
        return (<div className="flex flex-1 flex-col items-center justify-center gap-1.5 text-center text-leise">
    ...
    return (<div className="min-h-0 flex-1 overflow-auto">
      <table className="w-full min-w-[640px] border-collapse overflow-hidden rounded-knopf border border-white/10 bg-white/3 text-[13px] [&_tbody_tr:last-child_td]:border-b-0 [&_td:first-child]:min-w-[240px] [&_th:first-child]:min-w-[240px]">
        ...
        <tbody>
          {sichtbar.map((datei, index) => (<tr className="animate-gleiten [animation-delay:calc(var(--i)*10ms)] transition-colors hover:bg-neon-cyan/6" key={datei.relativer_pfad} style={{ "--i": Math.min(index, MAX_STAFFEL) }}>
              <td className={ZELLE}>
                <span className="font-mono text-[12px] text-neon-cyan">
                  {datei.relativer_pfad}
                </span>
                {datei.zeilen >= HOTSPOT_ZEILEN && (<span className="ml-2 inline-flex items-center gap-1.5 whitespace-nowrap rounded-full border border-warnung/40 px-2 py-0.5 text-[11px] font-medium text-warnung" title="Große Datei – Kandidat für einen Schnitt">
              ...
              <td className={ZAHL}>{(0, zahl_1.zahl)(datei.zeilen)}</td>
              <td className={ZAHL}>{(0, zahl_1.zahl)(datei.zeichen)}</td>
      ...
      {sichtbar.length < ergebnis.dateien.length && (<div className="flex flex-wrap items-center gap-3 px-2.5 py-3 text-[12px] text-leise">
          <span>
            Angezeigt: {(0, zahl_1.zahl)(sichtbar.length)} von {(0, zahl_1.zahl)(ergebnis.dateien.length)} Dateien –
            das Kontextpaket enthält alle.
          </span>
          <button type="button" onClick={() => setAlleZeigen(true)} className="rounded-knopf border border-neon-cyan/40 px-2.5 py-1 font-medium text-neon-cyan transition-colors hover:bg-neon-cyan/10">
            Alle {(0, zahl_1.zahl)(ergebnis.dateien.length)} Zeilen anzeigen
          </button>
        </div>)}
    </div>);
}
 ...
 */
exports.ErgebnisTabelle = (0, react_1.memo)(ErgebnisTabelleBasis);
