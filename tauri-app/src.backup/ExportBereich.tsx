...
const BESTANDTEILE: Array<[string, string]> = [
...
export function ExportBereich({ laedt, onSchreiben }: Props) {
  return (
    <div className="flex flex-col gap-3 border-t border-white/10 pt-3">
      <div className="flex flex-col gap-1.5">
        ...
        <ul className="flex flex-col gap-1">
          {BESTANDTEILE.map(([name, zweck]) => (
            <li
              className="grid grid-cols-[168px_1fr] items-baseline gap-2 text-[12px]"
              key={name}
            >
      ...
      <button
        onClick={onSchreiben}
        disabled={laedt}
        className="w-full rounded-knopf bg-gradient-to-r from-ok to-neon-cyan px-3.5 py-2.5 text-[14px] font-semibold text-[#052015] shadow-neon-gruen transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none"
      >
