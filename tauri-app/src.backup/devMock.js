...
function inhaltBauen(pfad, zeilen) {
    const anzahl = Math.min(zeilen, 12);
    return Array.from({ length: anzahl }, (_, i) => `// ${pfad} – Zeile ${i + 1}${i === anzahl - 1 ? ` von ${zeilen}` : ""}`).join("\n");
}
/**
 ...
 */
function beispieldateien(einstellungen) {
    ...
    if (einstellungen.maxDateien !== null && einstellungen.maxDateien < auswahl.length) {
        throw new Error(`Limit von ${einstellungen.maxDateien} Dateien erreicht – Abbruch vor "${auswahl[einstellungen.maxDateien]?.[0] ?? "…"}". Grenze erhöhen oder Limit entfernen.`);
    }
    const dateien = auswahl.map(([pfad, sprache, zeilen]) => ({
        relativer_pfad: pfad,
        zeilen,
        zeichen: zeilen * 38,
...
function mockScan(einstellungen, beiFortschritt) {
    let ergebnis;
    try {
...
function mockPaketSchreiben(ordner) {
    const domaenen = [
        ...new Set(BEISPIELDATEIEN.map(([pfad]) => (pfad.includes("/") ? pfad.split("/")[0] : "wurzel"))),
    ];
    const namen = [
        "Zusammenfassung.md",
        "Architektur.md",
...
const mockOrdnerWaehlen = async () => "C:/Beispiel/life-seed-lab";
exports.mockOrdnerWaehlen = mockOrdnerWaehlen;
