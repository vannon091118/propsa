...
exports.DeltaAnzeige = DeltaAnzeige;
const zahl_1 = require("./zahl");
function Liste({ titel, pfade, zeichen }) {
    if (pfade.length === 0) {
        return null;
    }
    return (<div>
      <div className="text-[11px] font-medium text-leise">
        {zeichen} {titel} ({(0, zahl_1.zahl)(pfade.length)})
      </div>
      <ul className="mt-1 max-h-28 overflow-auto font-mono text-[11px] leading-5">
        {pfade.slice(0, 20).map((pfad) => (<li key={pfad} className="truncate">
            {pfad}
          </li>))}
        {pfade.length > 20 && (<li className="text-leise">… und {(0, zahl_1.zahl)(pfade.length - 20)} weitere</li>)}
      </ul>
    </div>);
}
/**
 ...
 */
function DeltaAnzeige({ info }) {
    const kennung = info.identitaet.slice(0, 12);
    if (info.erstlauf || !info.delta) {
        return (<div className="animate-einfahren rounded-knopf border border-neon-cyan/35 bg-neon-cyan/8 px-3 py-2.5 text-[13px] text-tinte">
    ...
    return (<div className="animate-einfahren flex flex-col gap-2.5 rounded-knopf border border-neon-violett/35 bg-neon-violett/8 px-3.5 py-3">
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[13px]">
        ...
        <span className="text-tinte">
          <span className="text-ok">+{(0, zahl_1.zahl)(d.neu.length)}</span> ·{" "}
          <span className="text-warnung">~{(0, zahl_1.zahl)(d.geaendert.length)}</span> ·{" "}
          <span className="text-fehler">−{(0, zahl_1.zahl)(d.entfernt.length)}</span> ·{" "}
          <span className="text-leise">{(0, zahl_1.zahl)(d.unveraendert.length)} unverändert</span>
        </span>
      </div>

      {gesamt === 0 && (<div className="text-[12px] text-leise">
