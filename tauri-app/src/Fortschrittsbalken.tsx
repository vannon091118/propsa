...
export function Fortschrittsbalken({ fortschritt, sichtbar }: Props) {
  if (!sichtbar) {
    return null;
  }
  ...
  return (
    <div className="animate-einfahren flex flex-col gap-2 rounded-panel border border-white/10 bg-white/4 px-3.5 py-3">
      <div className="flex justify-between gap-3 text-[12px] text-leise">
        <span>
          {bekannt
            ? `${zahl(gelesen)} / ${zahl(gesamt)} Dateien · ${Math.round(anteil)} %`
            : "Sammle Dateien…"}
        </span>
      ...
      <div className="h-[7px] overflow-hidden rounded-full bg-white/6">
        <div
          className={`h-full rounded-full bg-gradient-to-r from-neon-cyan via-akzent to-neon-violett shadow-neon-cyan ${
            bekannt ? "" : "streifen-unbestimmt w-[35%] animate-schimmern"
          }`}
          style={bekannt ? { width: `${anteil}%` } : undefined}
        />
      </div>

      {fortschritt && (fortschritt.uebersprungen > 0 || fortschritt.zeilen > 0) && (
        <div className="flex justify-between gap-3 text-[12px] text-leise">
          <span>{zahl(fortschritt.zeilen)} Zeilen gelesen</span>
          <span>
