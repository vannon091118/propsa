    ...
    <section className="glas flex flex-col gap-3.5 overflow-auto rounded-panel p-4">
      ...
      <div className={FELD}>
        ...
        <input
          ...
          min={1}
          max={10000}
          value={alsFeldwert(einstellungen.maxDateien)}
          onChange={(e) =>
            onAendern({ maxDateien: e.target.value ? Number(e.target.value) : null })
          }
          className={KLEIN}
        />
      ...
      <div className={FELD}>
        ...
        <input
          ...
          min={1}
          max={1000000}
          value={alsFeldwert(einstellungen.maxZeilen)}
          onChange={(e) =>
            onAendern({ maxZeilen: e.target.value ? Number(e.target.value) : null })
          }
          className={KLEIN}
        />
      ...
      <div className={FELD}>
        ...
        <label className="flex cursor-pointer items-center gap-2.5 py-1.5">
          <input
            type="checkbox"
            checked={einstellungen.delta}
            onChange={(e) => onAendern({ delta: e.target.checked })}
      ...
      <div className={FELD}>
        ...
        <input
          type="text"
          value={einstellungen.includeMuster}
          onChange={(e) => onAendern({ includeMuster: e.target.value })}
          placeholder="z.B. src/**/*.ts, src/**/*.tsx"
          className={BREIT}
        />
      ...
      <div className={FELD}>
        ...
        <input
          type="text"
          value={einstellungen.excludeMuster}
          onChange={(e) => onAendern({ excludeMuster: e.target.value })}
          placeholder="node_modules, __pycache__, .cache, …"
          className={BREIT}
        />
        <div className={HINWEIS}>
        ...
        <button
          type="button"
          className="self-start text-[11px] text-neon-cyan underline decoration-dotted transition-colors hover:text-[#7ad9ee]"
          onClick={() => onAendern({ ...BASIS_EINSTELLUNGEN, pfad: einstellungen.pfad })}
        >
