...
fn reihenfolge_und_filter() {
    ...
    snapshot_schreiben(
        &verbindung,
        &tick("2026-09-16 08:00:00", 10, 100, 2),
        &signatur(10),
    )
    .expect("Snapshot 1");
    snapshot_schreiben(
        &verbindung,
        &tick("2026-09-16 09:30:00", 11, 120, 1),
        &signatur(11),
    )
    .expect("Snapshot 2");
    snapshot_schreiben(
        &verbindung,
        &tick("2026-09-17 08:00:00", 12, 150, 1),
        &signatur(12),
    )
    .expect("Snapshot 3");
    ...
    pruefe(
        "Ältester zuerst",
        alles.first().map(|p| p.zeitstempel.as_str()) == Some("2026-09-16 08:00:00"),
    );
    pruefe(
        "Metriken kommen an",
    ...
    // Zeitraum „letzte 20 Stunden“ (Referenz: 2026-09-17): nur der dritte.
    ...
    pruefe(
        "Gefiltert: der heutige Punkt",
        gefiltert[0].zeitstempel == "2026-09-17 08:00:00",
    );
    pruefe(
    ...
    pruefe(
        "Nach Kürzung: der letzte bleibt",
        nach_kuerzung[0].zeitstempel == "2026-09-17 08:00:00",
    );
}
...
fn kaputte_zeitstempel_kaufen_nicht_die_lesung() {
    ...
    snapshot_schreiben(
        &verbindung,
        &tick("2026-09-17 07:00:00", 5, 50, 0),
        &signatur(5),
    )
    .expect("Snapshot gut");
    ...
    pruefe(
        ...
        gefiltert.iter().all(|p| p.zeitstempel.as_str() >= "2026-09-17 06:00:00"),
    );
