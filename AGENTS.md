...
- `agents/shon-mcp-server/` ist ein optionales, eigenständiges stdio-MCP-Paket mit eigener Lockdatei; es gehört nicht zum Root-Workspace.
...
- `npm run pruefen` vergleicht tatsächlich Filterkataloge, Sprach-/Fence-Kataloge, Live-Kataloge sowie Cache-Version und -Schema; Schema, Domäne und History werden dort nicht automatisch verglichen.
...
## CI und aktuelle Skript-Blockaden
...
- Durch die aktuelle Skriptverschiebung sind `npm run changelog:spiegeln`, `npm run installieren`, `npm run validate` und die Setup-/Uninstall-Wrapper nicht zuverlässig: mehrere Skripte berechnen die Projektwurzel als `scripts/`, Wrapper rufen noch alte `scripts/install.mjs`-Pfade auf.
- `npm run architecture:check` ist unter Windows derzeit kein verlässliches Gate: `scripts/architecture-check.mjs` interpretiert den URL-Pfad als Windows-Pfad falsch. Vor Verwendung reparieren oder ausdrücklich als fehlerhaft kennzeichnen.
...
## Optionales Shinon-MCP

- Das Paket `agents/shon-mcp-server/` stellt einen stdio-MCP-Server mit den Tools `shinon_trigger_analysis`, `shinon_get_latest_snapshot`, `shinon_get_recent_explanations` und `shinon_get_status` bereit.
- Es legt `snapshots/repo_snapshots/`, `snapshots/diffs/` und `logs/` relativ zum Start-CWD an und schreibt dort; es ist nicht read-only. Ein Scheduler, `/heartbeat` oder `!shinon`-Kommando ist im Quelltext nicht vorhanden.
