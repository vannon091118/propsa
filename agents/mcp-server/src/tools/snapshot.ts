...
export const triggerAnalysisTool = {
  name: "shinon_trigger_analysis",
  description: "Manually trigger a SHINON repository snapshot and generate analysis explanation",
  ...
  execute: async (args: { force?: boolean }) => {
    try {
      ...
      const config = configRaw as { shinon?: any; repository?: any };
      ...
      snapshotLines.push("# Repository-Snapshot (SHINON)");
      ...
      explanationLines.push("# SHINON Analyse-Erklärung");
      ...
      explanationLines.push("");
      explanationLines.push("---");
      explanationLines.push("*Diese Erklärung wurde automatisch von SHINON generiert.*");
      ...
      let diffReport = "# Change Report\n\n**Vergleich**: SHINON Analysis Execution\n\n";
      ...
      // Log the activity
      const logEntry = `${new Date().toISOString()} | SHINON Manual Trigger | ${scanResult.totalFiles} files | ${scanResult.totalLines} LOC\n`;
      appendTextFile(path.join(LOGS_DIR, "shinon.log"), logEntry);
      
      return {
        content: [
          {
            type: "text",
            text: `✅ SHINON Analysis Triggered Successfully!\n\n📊 Snapshot erstellt: ${snapshotId}\n📁 Dateien: ${scanResult.totalFiles}\n🔢 LOC: ${scanResult.totalLines}\n😊 Fun-Message: ${funMessages[funIdx]}\n\n📁 Dateien erstellt:\n- Snapshot: ${snapshotPath}\n- Erklärung: ${explPath}\n- Diff-Report: ${diffPath}\n\nDie Analyse ist jetzt über die SHINON-MCP-Tools verfügbar.`
          }
        ]
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `❌ SHINON Analysis Failed: ${error instanceof Error ? error.message : String(error)}`
          }
        ],
        isError: true
      };
