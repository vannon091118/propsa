...
export const getRecentExplanationsTool = {
  name: "propakt_get_recent_explanations",
  description: "Get recent PROPAKT explanations (German with fun messages)",
  ...
  execute: async (args: { limit?: number }) => {
    try {
      ...
      if (!fs.existsSync(DIFF_DIR)) {
        return {
          content: [
            {
              type: "text",
              text: "📭 Noch keine Erklärungen vorhanden. Führen Sie zuerst 'propakt_trigger_analysis' aus."
            }
          ]
        };
      }
      ...
      let response = `📝 Letzte ${explanations.length} PROPAKT Erklärungen:\n\n`;
